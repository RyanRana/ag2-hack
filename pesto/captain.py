"""PestoCaptain — top-level orchestrator (§8.7).
...
from pesto.cross_exam import cross_examine, disputed_plants
from pesto.detection import detect_plants_yolo
from pesto.latent import CONDITION_LABELS, FieldLatentState, INTERVENTION_TYPES
...
# Per-action default chemistry. The point of Pesto is to show what *would*
...
class PestoCaptain:
    ...
    def __init__(
        ...
        *,
        ...
        chemistry_per_action: dict[str, str] | None = None,
    ) -> None:
        ...
        self.interventions = list(interventions) if interventions else list(INTERVENTION_TYPES)
        self.chemistry_per_action = {
            **DEFAULT_CHEMISTRY_PER_ACTION,
            **(chemistry_per_action or {}),
        }
        ...
        *,
        ...
        for plant in latent.plants:
            ...
            for action in self.interventions:
                ...
                chem = override or self.chemistry_per_action.get(action, "glyphosate")
    ...
    def run_swarm_inference(self, *args, **kwargs) -> dict:
        ...
        ``pesto.runtime.wire_swarm_pipeline`` runs a Swarm-driven
