...
Enhanced to support two inference paths:
  ...
  1. **Local VLM** (preferred): LLaVA/InternVL2 loaded via pesto.local_model.
...
``pesto.local_model.get_shared_backend()``.
"""
...
from pesto.llm_config import llm_key_available, openai_llm_config
...
class VLMReasonerAgent(AssistantAgent):
    ...
    When a local model backend is available (via pesto.local_model), the agent
