...
from pesto.agents.anomaly_detector import AnomalyDetectorAgent
from pesto.agents.disease_classifier import DiseaseClassifierAgent
from pesto.agents.growth_stage import GrowthStageAgent
from pesto.agents.health_classifier import HealthClassifierAgent
from pesto.agents.segmentation import SegmentationAgent
from pesto.agents.weather_prior import WeatherPriorAgent
from pesto.agents.weed_detector import WeedDetectorAgent
from pesto.captain import PestoCaptain
from pesto.cross_exam import max_disagreement_per_plant
...
def create_app() -> FastAPI:
    app = FastAPI(title="Pesto Precision Agriculture")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
        allow_methods=["*"],
...
def _build_captain(emit_event=None, *, live: bool = False) -> PestoCaptain:
    ...
    if not live and llm_key_available():
        ...
        from pesto.agents.vlm_reasoner import VLMReasonerAgent
...
def _post_inference_hooks(
    ...
    if seg_agent and getattr(seg_agent, "evidence_maps", None):
        try:
            ...
            from pesto.visual_explain import render_field_explanation
    ...
    try:
        if state.get("rag") is None:
            from pesto.rag.retriever import AgronomicRAG
    ...
    if al_mgr:
        try:
            ...
            from pesto.messages import ActionMessage as AM
    ...
    if prev_path and Path(src_path).exists() and Path(prev_path).exists():
        try:
            from pesto.temporal import compute_frame_diff
