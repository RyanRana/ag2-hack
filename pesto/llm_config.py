...
def openai_llm_config(*, model: str | None = None, temperature: float = 0.0) -> dict:
    ...
    OpenAI is used. Pesto's Skeptic + VLMReasoner work identically against
...
def build_llm_config(*, model: str | None = None, temperature: float = 0.0) -> dict:
    ...
    override_model = model or os.environ.get("PESTO_LLM_MODEL")
    ...
    raise LLMKeyMissingError(
        "Neither ANTHROPIC_API_KEY nor OPENAI_API_KEY is set. Pesto's "
